...
### 07_reclean_migration_headers.py (196 LOC)

**Purpose:** Re-clean existing issue MDs — strip the tracker-migration attribution header + rule (junk class F): a bold `**[Original report](bitbucket_url) by NAME (...).**` line (issue body, first content) or `**Original comment by NAME (...).**` line (each migrated comment), always followed by a blank line then exactly 40 dashes. Combines the equivalent of `strip_noise`'s and `strip_comments_noise`'s class-F branches into one pass (the two anchors never collide within a single already-assembled MD). Dry-run by default; `--apply` overwrites in place after a timestamped backup. Dry-run against the live corpus (2026-09-05, 963 files): 7 files changed (exactly `pyobjc__175/176/31/34/44/76/77.md`; `pyobjc__610.md`, a native GitHub issue, unaffected), 36 spans, 108 lines, 7,628 chars removed — no `--apply` run, see `process-docs/content_cleaning/`.
**Reads:** issue MD corpus (`--source-dir PATH` override); verbatim inline copy of `src/github/index_issues.py`'s class-F anchors + removal logic.
**Writes:** report MD to `md/07_reclean_dryrun_<timestamp>.md` — summary numbers, a per-file table, then the verbatim removed text for every span (one `<file>:<start>-<end>` identification line per span, then the text, nothing else) so the whole removal can be read in full; with `--apply`, backs up the full corpus first to `<source_dir>_PRE_MIGRATION_HEADER_STRIP_BACKUP_<timestamp>` (not disposable, do not delete), then overwrites only the changed corpus files; prints the report path and backup path.
**Called by:** run manually (dev entry point).
**Calls out:** stdlib only.
...
- `07_reclean_migration_headers.py` contains an intentional verbatim copy of `src/github/index_issues.py`'s class-F anchors (`MIGRATION_REPORT_RE`, `MIGRATION_COMMENT_RE`, `MIGRATION_RULE_RE`) and removal logic (`_find_migration_blocks`/`strip_migration_header`, a `strip_build_logs`-shaped block-and-splice, no placeholder). Source of truth: `src/github/index_issues.py`. Unlike `strip_noise`/`strip_comments_noise`, this script runs the class-F removal on the whole already-built MD in one pass rather than on body/comments separately — safe because the two anchor forms never occur outside their respective section and match nothing else observed in the corpus.
